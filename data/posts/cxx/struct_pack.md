...
# struct_pack代码解读
[struct_pack](https://alibaba.github.io/yalantinglibs/en/struct_pack/struct_pack_intro.html#serialization)通过编译期反射自动支持对C++结构体的序列化和反序列化。同时使用大量的模板元编程，是学习模板元编程的好项目。
## 关键流程解读
### 使用示例
```C++
struct person 
...
通过简单的调用struct_pack::serialize进行序列化。serialize中又调用了serialize_to。这里的serialize_to主要有两个功能：

1. 是获取序列化信息`detail::get_serialize_runtime_info`。
2. 是真正的序列化写入`serialize_to<conf>(writer, info, args...)`
...
该函数中首先通过`calculate_payload_size`获取`size_info`信息，然后对`size_info`进行修正完善。这里`size_info`信息时通过`visit_members`获取自定义结构体的信息。`visit_members`函数中通过`member_count_impl`在编译期获取自定义结构体的成员变量个数。这里有个小技巧，聚合类 Foo满足以下所有的初始化方法:
```C++
struct Foo {
    int a;
    int b;
...
所以`member_count_impl`的核心思想是通过不断加参数直到构造成功，最后返回成功时的参数列表个数。
```C++
...
而后通过结构化绑定将参数取出，最后通过自定义的`visitor`获取想要的信息,这里是获取`sizecalculate_payload_size`。`get_serialize_runtime_info`的后半部分重新更新序列化数据的信息。
