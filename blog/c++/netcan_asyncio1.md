...
## asyncio::Task
以```asyncio::run(coro_depth_n<1>(result))```为例,根据[c++20_Coroutines入门](http://www.aiecent.com/articleDetail?article_id=47)所述步骤编译器将代码展开为如下形式：
```C++
...
* 说明1
执行完后挂起，返回task协程对象。通过schedule加入到协程ready_队列，而后通过run_until_complete函数resume协程运行try中的body。
