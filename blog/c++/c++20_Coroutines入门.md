...
## 简介
&#8195;&#8195;功能与异步编程相似不阻塞程序运行。可以在某个地方挂起，并且可以重新在挂起处恢复运行。  
&#8195;&#8195;协程与线程最大不同之处在于调度模式。线程通过操作系统切换内核栈上下文切换线程，从而执行不同的函数。协程通过编程者切换自己保存的用户栈切换协程任务，从而执行不同的函数。

### 适用场景
* 回调多的地方，用同步的方法写异步函数。c++11中通过std::packaged_task，std::promise也能实现类似的功能。[c++11适用同步方式写一遍代码](https://github.com/BestUO/littletools/blob/master/tooltest.cpp)，参考main中的testThreadpool函数。
```c++
//callback方式:
{
	requestA(1, [](int rt)
...
//c++11适用同步方式写一遍代码:
{
	std::future<int> rt = pool.enqueue(requestA(1));
	rt = pool.enqueue(requestB(rt.get()));
	rt = pool.enqueue(requestC(rt.get()));
}
```
* io密集型业务场景，这是协程最大的优势了。可以在等待io的时候暂停该协程，处理下一个协程任务。待io完成之后继续处理未完成的协程任务，因此性能很高。[libgo](https://github.com/yyzybb537/libgo)中采用的方式是hook系统函数比如poll，read，write，accept等，然后加入到自己的epoll中。类似于这样：
```c++
...
## demo
&#8195;&#8195;[c++20协程简单demo](https://github.com/BestUO/littletools/tree/master/coroutinestest)


## 参考
[c++20协程入门](https://zhuanlan.zhihu.com/p/59178345)  
[C++20 协程初探](https://netcan.github.io/2020/09/05/C-20%E5%8D%8F%E7%A8%8B/)  
...
[协程简介](https://lewissbaker.github.io/)  
[[译]C++ 协程 1：协程理论](https://blog.csdn.net/sinat_17700695/article/details/115770430?spm=1001.2014.3001.5502)  
[[译]C++ 协程 2：理解 co_await 运算符](https://juejin.cn/post/6844903715099377672)  
[[译]C++ 协程 3：理解 promise type](https://blog.csdn.net/sinat_17700695/article/details/116016136)  
[Understanding C++ coroutines by example](https://downloads.ctfassets.net/oxjq45e8ilak/4kVoTkxYBiVM9lPBZiG2HO/a5e36dc80fd898885269bd6320c96196/Pavel_Novikov_Uchimsya_gotovit_C_korutiny_na_praktike_2020_06_28_18_49_49.pdf)  
[C++20 Coroutine: Under The Hood](http://www.vishalchovatiya.com/cpp20-coroutine-under-the-hood/)  
[C++ coroutines: Short-circuiting suspension](https://devblogs.microsoft.com/oldnewthing/20191216-00/?p=103217)  
[C++20 Coroutines — Complete Guide](https://itnext.io/c-20-coroutines-complete-guide-7c3fc08db89d)  
[C++20 — Practical Coroutines](https://itnext.io/c-20-practical-coroutines-79202872ebba)  
[协程池实现](https://zhuanlan.zhihu.com/p/375279181)  
[使用c++20协程和epoll封装简单网络库(一) 使用协程](https://zhuanlan.zhihu.com/p/430014469)  
[使用c++20协程和epoll封装简单网络库(二) co_await](https://zhuanlan.zhihu.com/p/432627058)  
